...
## Styling the Accessibility Tree

Prefer the attribute the platform already exposes. Do not mirror it into `data-*`.

```html
<button class="menu_button" type="button" aria-expanded="true" aria-controls="menu">Filters</button>
```
...
.menu_button[aria-expanded='true'] .menu_button-chevron {
  /* Wrong: descendant selector. Route the state through a custom property. */
}
```

```css
/* Correct: the parent publishes state as a property, the child reads it. */
.menu_button {
  --c-chevron-turn: 0deg;
}

.menu_button[aria-expanded='true'] {
  --c-chevron-turn: 180deg;
}

.menu_button-chevron {
  rotate: var(--c-chevron-turn);
